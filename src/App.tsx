...
export default function App(){
 ...
 const runExecution=async()=>{await runBuilderExecutionPreview(buildInput || 'Builder task')};
 const submit=(e:any)=>{e.preventDefault();if(!buildInput.trim())return;const text=buildInput.trim();setMessages(m=>[...m,{role:'user',content:text},{role:'assistant',content:'Added to build context.'}]);setBuildInput('')};

 function panel(){switch(workspaceTab){case 'preview': return <WorkspacePreviewPanel/>;case 'files': return <ProjectFilesPanel projectId={activeProjectId}/>;case 'commits': return <CommitPanel projectName={activeProject?.name ?? 'Workspace'}/>;default:return <section className='workspace-content builder-workspace'><div className='ai-conversation'><div className='ai-message assistant-message'><strong>Vivus</strong><p>{activeProject ? `${activeProject.name} is loaded. What should we build first?` : 'What would you like to build today?'}</p></div>{messages.map((m,i)=><div key={i} className={`ai-message ${m.role==='user'?'user-message':'assistant-message'}`}><strong>{m.role==='user'?'You':'Vivus'}</strong><p>{m.content}</p></div>)}<form onSubmit={submit} className='ai-builder-input-form bottom-composer'><textarea value={buildInput} onChange={(e)=>setBuildInput(e.target.value)} placeholder='Make, test, iterate...' /><button type='button' className='ai-builder-plus'><Plus size={18}/></button><div className='composer-controls'><button type='button' className='soft-button' onClick={runExecution}>Preview</button><button type='submit' className='send-button'><Send size={16}/></button></div></form></div></section>}}
