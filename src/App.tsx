...
type Task={title:string,status:'queued'|'running'|'done'};
...
export default function App(){
 ...
 const runExecution=async()=>{const result=await runBuilderExecutionPreview(buildInput || 'Builder task');setActivity(a=>[...a,result.message])};
 const submit=(e:any)=>{e.preventDefault();if(!buildInput.trim())return;const text=buildInput.trim();setTasks([{title:'Analyze request',status:'done'},{title:'Source verification',status:'running'},{title:'Apply patch',status:'queued'},{title:'Verification',status:'queued'}]);setActivity(['Plan generated','Awaiting approval']);setMessages(m=>[...m,{role:'user',content:text},{role:'assistant',content:mode==='plan'?'Build plan generated. Review task queue below.':'Added to build context.'}]);setBuildInput('')};

 function panel(){switch(workspaceTab){case 'preview': return <WorkspacePreviewPanel/>;case 'files': return <ProjectFilesPanel projectId={activeProjectId}/>;case 'commits': return <CommitPanel projectName={activeProject?.name ?? 'Workspace'}/>;default:return <section className='workspace-content builder-workspace'><div className='ai-conversation'><div className='ai-message assistant-message'><strong>Vivus</strong><p>{activeProject ? `${activeProject.name} is loaded. What should we build first?` : 'What would you like to build today?'}</p></div>{messages.map((m,i)=><div key={i} className={`ai-message ${m.role==='user'?'user-message':'assistant-message'}`}><strong>{m.role==='user'?'You':'Vivus'}</strong><p>{m.content}</p></div>)}{!!tasks.length && <section className='builder-workflow-panel'><h3>Task Queue</h3>{tasks.map((t,i)=><div key={i}><strong>{t.status}</strong> {t.title}</div>)}<h3>Activity</h3>{activity.map((a,i)=><div key={i}>{a}</div>)}</section>}<form onSubmit={submit} className='ai-builder-input-form bottom-composer'><textarea value={buildInput} onChange={(e)=>setBuildInput(e.target.value)} placeholder={mode==='plan'?'Describe requirements and goals...':'Make, test, iterate...'} /><button type='button' className='ai-builder-plus'><Plus size={18}/></button><div className='composer-controls'><button type='button' className='mode-selector' onClick={()=>setMode(m=>m==='build'?'plan':'build')}><span>{mode==='build'?'Build':'Plan'}</span><ChevronDown size={12}/></button><button type='button' className='soft-button' onClick={runExecution}>Preview</button><button type='submit' className='send-button'><Send size={16}/></button></div></form></div></section>}}
